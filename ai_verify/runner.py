...
典型用法：
  ...
  ai-verify run --port 8080 -- python my_script.py
...
from ai_verify.providers.cc_switch import ProviderConfig, resolve_provider
...
def _start_proxy_thread(host: str, port: int, provider: ProviderConfig) -> threading.Thread:
    ...
    t = threading.Thread(target=_run, name="ai-verify-proxy", daemon=True)
    t.start()
    return t
...
def run_with_proxy(
    ...
    """
    if not command:
        console.print("[red]✗ 请在 -- 后面写要运行的命令，例如: ai-verify run -- claude[/red]")
        return 2

    provider = resolve_provider(prefer=prefer)
    if not provider:
        console.print("[red]✗ 未找到上游配置[/red]")
        console.print("  请在 CC Switch 里选一个供应商，或: [cyan]ai-verify config set base_url <url>[/cyan]")
        return 1
    ...
    try:
        ...
        if not quiet:
            console.print(
                "\n[dim]命令已结束，监控代理随之退出。查看记录: ai-verify history --last 1d[/dim]"
            )
