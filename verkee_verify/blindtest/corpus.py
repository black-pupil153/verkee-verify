...
from verkee_verify.cursor_logs import iter_log_events
...
def build_label_index(
    ...
    verkee_verify_db: Optional[Path] = None,
    ...
    from verkee_verify.cursor_hook_events import default_hook_event_paths, iter_hook_events
    ...
    if hook_event_paths is not None:
        ...
        # Fixture / explicit path mode — do not pull host ~/.verkee/verify hooks.
        paths = []
    ...
    log_files: List[Path] = []
    if logs_dir is None:
        from verkee_verify.providers.cursor import discover_cursor_paths, probe_structured_logs
    ...
    if tracking_db is None:
        from verkee_verify.providers.cursor import discover_cursor_paths

        tracking_db = discover_cursor_paths().ai_tracking_db
    if tracking_db and Path(tracking_db).is_file():
        from verkee_verify.providers.cursor import read_ai_code_hashes
    ...
    # 3. 可选：verkee-verify DB 里 confidence 较高的 resolved_model
    if verkee_verify_db and Path(verkee_verify_db).is_file():
        import sqlite3

        try:
            with sqlite3.connect(verkee_verify_db) as conn:
                rows = conn.execute(
                    """
...
def build_corpus(
    ...
    index = build_label_index(
        ...
        verkee_verify_db=verkee_verify_db,
