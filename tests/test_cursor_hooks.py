...
def test_merge_hooks_preserves_existing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "ai_verify.cursor_hooks.hook_script_path",
...
def test_install_and_uninstall_hooks(tmp_path, monkeypatch):
    ...
    script_dir = tmp_path / "ai-verify" / "hooks"
    monkeypatch.setattr("ai_verify.cursor_hooks.cursor_hooks_json_path", lambda: hooks_json)
    monkeypatch.setattr(
        "ai_verify.cursor_hooks.hook_script_path",
        lambda: script_dir / "cursor-track.sh",
    )
    monkeypatch.setattr(
        "ai_verify.cursor_hooks.ai_verify_home",
        lambda: tmp_path / "ai-verify",
    )
