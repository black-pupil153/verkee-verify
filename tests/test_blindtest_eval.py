...
from verkee_verify.blindtest.corpus import Corpus, CorpusSample, exclude_conversations
...
def test_run_artifacts_no_prompt_text(tmp_path: Path, monkeypatch):
    from verkee_verify.blindtest.splits import SealedLabel

    monkeypatch.setattr(
        "verkee_verify.blindtest.eval.DEFAULT_BLINDTEST_DIR", tmp_path
    )
    corpus = _build_corpus()
...
def test_auto_eval_agree_and_opaque():
    ...
    from verkee_verify.blindtest.eval import evaluate_auto_reports
...
def test_apply_channels_drops_latency():
    from verkee_verify.blindtest.eval import apply_channels_to_samples
...
def test_near_gate_eval_matches_predict_and_swap_metric():
    from verkee_verify.blindtest.classifier import (
        ABSTAIN_NEAR_MARGIN,
        select_inferred_model,
    )
    from verkee_verify.blindtest.eval import evaluate_predictions
