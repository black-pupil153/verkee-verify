...
from verkeep_verify.cursor_hooks import hooks_installed
from verkeep_verify.monitor.cursor_usage import PeriodUsageReport, aggregate_period, parse_since
...
def build_recommendations(
    ...
    if rate >= RESOLUTION_THRESHOLD:
        lines.append(
            ...
            "继续用 verkeep-verify cursor task --latest 观察即可。"
        )
        return lines
    ...
    if not hooks_ok:
        ...
        lines.append("  [cyan]verkeep-verify cursor hooks install[/cyan]")
        lines.append("  安装后请完全重启 Cursor，再跑 1 次 Auto Agent turn，然后：")
        lines.append("  [cyan]verkeep-verify cursor hooks analyze[/cyan]")
    else:
    ...
    lines.append("  验证: [cyan]verkeep-verify cursor import --since 1d && verkeep-verify cursor task --latest[/cyan]")
    ...
    if not _blindtest_model_exists():
        ...
        lines.append("  [cyan]verkeep-verify blindtest build-corpus && verkeep-verify blindtest train[/cyan]")
        ...
        lines.append(f"  [cyan]verkeep-verify cursor task {hint} --inferred[/cyan]")
...
def show_recommendations(since: str = "7d") -> None:
    ...
    if period.task_count == 0:
        console.print("[yellow]暂无数据，请先运行: verkeep-verify cursor import --since 7d[/yellow]")
        return
