...
Does not invent Auto GT. Uses real ~/.ai-verify DB + CLI contracts.
"""
...
def _ai_verify(*args: str, timeout: float = 30.0) -> subprocess.CompletedProcess:
    exe = REPO / "venv" / "bin" / "ai-verify"
    cmd = [str(exe) if exe.is_file() else "ai-verify", *args]
    return _run(cmd, timeout=timeout)


def check_doctor() -> Check:
    r = _ai_verify("cursor", "doctor", "--json")
    if r.returncode != 0:
...
def check_latest_mix() -> Check:
    ...
    r = _ai_verify("cursor", "task", "--latest", "--json")
...
def check_human_default() -> Check:
    r = _ai_verify("cursor", "task", "--latest")
    if r.returncode != 0:
...
def check_list_detail_parity() -> Check:
    r = _ai_verify("cursor", "tasks", "--json")
    if r.returncode != 0:
    ...
    for t in ordered[:5]:
        tid = t["task_id"]
        dr = _ai_verify("cursor", "task", tid, "--json")
        if dr.returncode != 0:
