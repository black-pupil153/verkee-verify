...
from ai_verify.dashboard import run_scoring
...
def _maybe_alert(snapshot: dict):
    ...
    try:
        from ai_verify.alerts import AlertLevel, AlertManager
...
def run(interval: str = "6h", num_probes: int = 6, num_questions: int = 8) -> None:
    ...
    if not provider:
        console.print("[red]✗ 未找到上游配置（CC Switch / Claude settings / ai-verify config）[/red]")
        return
