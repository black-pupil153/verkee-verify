...
from verkeep_verify.dashboard import run_scoring
...
def _maybe_alert(snapshot: dict):
    ...
    try:
        from verkeep_verify.alerts import AlertLevel, AlertManager
...
def run(interval: str = "6h", num_probes: int = 6, num_questions: int = 8) -> None:
    ...
    if not provider:
        console.print("[red]✗ 未找到上游配置（CC Switch / Claude settings / verkeep-verify config）[/red]")
        return
