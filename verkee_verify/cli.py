...
from verkee_verify import __version__
...
@click.group()
@click.version_option(version=__version__, prog_name="verkee-verify")
def main():
    """
    Verkee Verify - AI API 实时监控工具

    给你的 AI API 装一个监控摄像头，实时检测模型真假、质量变化和异常行为。
    """
    # Migrate legacy ~/.ai-verify to ~/.verkee/verify on startup
...
def config_init():
    ...
    manager.init_config()
    console.print("[green]✓[/green] 配置文件已创建: ~/.verkee/verify/config.yaml")
...
def config_set(key: str, value: str):
    ...
    示例:
        verkee-verify config set base_url https://api.example.com/v1
        verkee-verify config set api_key sk-xxx
        verkee-verify config set model gpt-4
    """
    from verkee_verify.config import ConfigManager
...
def proxy_start(port: int, host: str):
    """启动代理服务器"""
    from verkee_verify.proxy.server import start_proxy_server
...
def proxy_status():
    ...
    console.print("使用 [cyan]verkee-verify proxy start[/cyan] 启动")
...
def monitor_start(interval: str, probes: int, questions: int):
    ...
    from verkee_verify.monitor.daemon import run
...
def providers_list():
    ...
    from verkee_verify.providers.cc_switch import (
...
@click.option(
    ...
    "prefer",
    default="auto",
    type=click.Choice(["auto", "cc-switch", "claude", "verkee-verify"]),
...
def run(ctx, port: int, host: str, quiet: bool, prefer: str):
    ...
    示例:
      ...
      verkee-verify run --port 9090 -- claude
      verkee-verify run --from verkee-verify -- python my_script.py
    """
    import sys

    from verkee_verify.runner import run_with_proxy

    command = list(ctx.args)
    if not command:
        console.print("[red]✗ 请在 -- 后面写命令[/red]")
        console.print("  示例: [cyan]verkee-verify run -- claude[/cyan]")
        sys.exit(2)
...
def check(full: bool, model, probes: int, questions: int, prefer: str):
    ...
    if not provider:
        ...
        console.print(
            "  请在 CC Switch 选一个供应商，或: verkee-verify config set base_url <url>"
        )
        return
...
def score(
    ...
    from verkee_verify import dashboard
...
def alert_test(webhook):
    """测试报警功能"""
    from verkee_verify.alerts import AlertManager, AlertLevel
    ...
    # 发送测试报警
    manager.alert(
        title="Verkee Verify 测试报警",
        message="这是一条测试报警消息，验证报警功能是否正常工作。",
        level=AlertLevel.WARNING,
        details={
            "测试时间": "现在",
            "来源": "verkee-verify test",
        },
    )
...
def alert_set(webhook_url: str, platform: str):
    ...
    console.print()
    console.print("[dim]测试报警: verkee-verify alert test[/dim]")
...
def report(period: str):
    """生成监控报告"""
    from verkee_verify import report as report_mod
...
def cursor(ctx, since: str):
    ...
    if ctx.invoked_subcommand is None:
        from verkee_verify.dashboard_cursor import show_board
...
def cursor_report(period: str, no_refresh: bool):
    ...
    from verkee_verify.report import generate_cursor
...
def cursor_board(
    ...
    from verkee_verify.dashboard_cursor import show_board, watch_board
...
def cursor_doctor(as_json: bool):
    ...
    from verkee_verify.providers.cursor import run_doctor
...
def cursor_import(since: str, full: bool):
    ...
    from verkee_verify.monitor.cursor_usage import CursorUsageImporter, parse_since
    ...
    console.print(f"\n[dim]查看: verkee-verify cursor --since {since}[/dim]")
...
def cursor_tasks(since: str, limit: int, auto_only: bool, as_json: bool):
    ...
    if not items:
        console.print("[yellow]暂无任务，请先运行: verkee-verify cursor import[/yellow]")
        return
...
def cursor_task(
    ...
    if verbose:
        ...
        if report.inferred_request_shares:
            ...
            console.print(
                ...
                "逐 turn 细节: verkee-verify cursor task --inferred[/yellow]"
            )
    ...
    if score and (report.output_shares or report.request_shares):
        ...
        from verkee_verify.monitor.cursor_usage import get_model_scores
        ...
        if any(not scores.get(m) for m in score_models):
            console.print(
                "[dim]部分模型无智力分记录，可对当前渠道运行 verkee-verify score 补测[/dim]"
            )
...
def _render_inferred_view(db, task_id: str, console) -> None:
    ...
    if view is None:
        from verkee_verify.monitor.blindtest_infer import DEFAULT_MODEL_PATH
        ...
        if not DEFAULT_MODEL_PATH.is_file():
            console.print(
                "[dim]无推断记录，且未找到训练模型。"
                "先运行 verkee-verify blindtest train[/dim]"
            )
        else:
...
def cursor_hooks_install():
    ...
    from verkee_verify.cursor_hooks import install_hooks
    ...
    console.print("之后运行: [cyan]verkee-verify cursor hooks analyze[/cyan]")
...
def cursor_hooks_analyze():
    ...
    if not path.is_file():
        ...
        console.print(
            "请先运行: verkee-verify cursor hooks install，重启 Cursor 并执行一次 Agent turn"
        )
        return
...
def cursor_hooks_uninstall():
    """从 hooks.json 移除 verkee-verify 探针条目"""
    from verkee_verify.cursor_hooks import uninstall_hooks

    hooks_path, backup, removed = uninstall_hooks()
    if not removed:
        console.print("[yellow]未找到 verkee-verify hook 条目[/yellow]")
        return
    ...
    console.print(
        "[dim]探针脚本 ~/.verkee/verify/hooks/cursor-track.sh 仍保留，可手动删除[/dim]"
    )
...
def cursor_probe(task_id: Optional[str], limit: int):
    ...
    from verkee_verify.cursor_probe import format_probe_report, run_probe
...
def cursor_recommend(since: str):
    ...
    from verkee_verify.cursor_recommend import show_recommendations
...
def blindtest_build_corpus(since: Optional[str], include_unlabeled: bool):
    ...
    from verkee_verify.blindtest.corpus import build_corpus, save_corpus
...
def blindtest_inventory(split_name: Optional[str], long_min: int, targets: str, as_json: bool):
    ...
    from verkee_verify.blindtest.corpus import load_corpus
    from verkee_verify.blindtest.inventory import build_inventory

    try:
        corpus = load_corpus()
    except FileNotFoundError:
        console.print("[red]✗ 未找到语料，请先运行 verkee-verify blindtest build-corpus[/red]")
        raise SystemExit(1)

    split = None
    if split_name:
        from verkee_verify.blindtest.splits import load_split
...
def blindtest_split(name: str, seed: int, ratios: str):
    ...
    from verkee_verify.blindtest.corpus import load_corpus
    from verkee_verify.blindtest.splits import (
        create_split,
...
def blindtest_train(
    ...
    from verkee_verify.blindtest.corpus import load_corpus
    from verkee_verify.blindtest.eval import (
        apply_channels_to_corpus,
        new_run_dir,
        save_run_artifacts,
    )
    from verkee_verify.blindtest.features import parse_channels
    ...
    if split_name is not None:
        from verkee_verify.blindtest.splits import load_split, strip_test_labels
        ...
        try:
            ...
            console.print(
                ...
                f"请先运行 verkee-verify blindtest split --name {split_name}[/red]"
            )
            raise SystemExit(1)
...
def blindtest_eval(
    ...
    if not model_path.is_file():
        console.print("[red]✗ 未找到模型，请先运行 verkee-verify blindtest train --split …[/red]")
        raise SystemExit(1)
...
def blindtest_infer(task_id: str, save_db: bool):
    ...
    model_path = _blindtest_model_path()
    if not model_path.is_file():
        console.print("[red]✗ 未找到模型，请先运行 verkee-verify blindtest train[/red]")
        raise SystemExit(1)
    clf = BlindModelClassifier.load(model_path)
