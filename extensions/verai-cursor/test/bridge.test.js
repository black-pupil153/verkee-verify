...
function bridgeWithResponses(responses) {
  ...
  return { bridge: new AiVerifyBridge(() => "/custom/verkee-verify", runner), calls };
}
...
test("doctor JSON contract and command", async () => {
  ...
  assert.equal(calls[0].executable, "/custom/verkee-verify");
...
test("recent import uses configured executable without requiring Cursor data", async () => {
  ...
  const bridge = new AiVerifyBridge(() => "/configured/verkee-verify", runner);

  await bridge.importRecent("1d");
  assert.equal(calls[0].executable, "/configured/verkee-verify");
...
test("invalid JSON is classified", async () => {
  ...
  const bridge = new AiVerifyBridge(() => "verkee-verify", runner);
