...
def hook_script_path() -> Path:
    return verify_home() / "hooks" / HOOK_SCRIPT_NAME


def probe_ndjson_path() -> Path:
    return verify_home() / "cursor-hook-probe.ndjson"
...
# verkeep-verify Cursor hook probe — append full stdin JSON to ndjson log
set -euo pipefail
EVENT="${1:-unknown}"
NDJSON="${HOME}/.verkeep/verify/cursor-hook-probe.ndjson"
