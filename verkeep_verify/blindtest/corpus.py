...
from verkeep_verify.cursor_logs import iter_log_events
from verkeep_verify.paths import verify_home

DEFAULT_BLINDTEST_DIR = verify_home() / "blindtest"
...
def build_label_index(
    ...
    verkeep_verify_db: Optional[Path] = None,
    ...
    from verkeep_verify.cursor_hook_events import default_hook_event_paths, iter_hook_events
    ...
    if hook_event_paths is not None:
        ...
        # Fixture / explicit path mode — do not pull host ~/.verkeep/verify hooks.
        paths = []
    ...
    log_files: List[Path] = []
    if logs_dir is None:
        from verkeep_verify.providers.cursor import discover_cursor_paths, probe_structured_logs
    ...
    if tracking_db is None:
        from verkeep_verify.providers.cursor import discover_cursor_paths

        tracking_db = discover_cursor_paths().ai_tracking_db
    if tracking_db and Path(tracking_db).is_file():
        from verkeep_verify.providers.cursor import read_ai_code_hashes
    ...
    # 3. 可选：verkeep-verify DB 里 confidence 较高的 resolved_model
    if verkeep_verify_db and Path(verkeep_verify_db).is_file():
        import sqlite3

        try:
            with sqlite3.connect(verkeep_verify_db) as conn:
                rows = conn.execute(
                    """
...
def build_corpus(
    ...
    index = build_label_index(
        ...
        verkeep_verify_db=verkeep_verify_db,
