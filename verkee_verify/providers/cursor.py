...
def _open_sqlite_readonly(db_path: Path) -> sqlite3.Connection:
    ...
    tmp = Path(tempfile.gettempdir()) / f"verkee-verify-cursor-{db_path.name}"
...
def run_doctor(verkee_verify_db: Optional[Path] = None) -> DoctorReport:
    ...
    verify_db = verkee_verify_db or (verify_home() / "data" / "ai_verify.db")
    ...
    report.suggestion = "verkee-verify cursor import --since 7d"
    return report
