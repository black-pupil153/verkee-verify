...
现有 [`features.py`](../../ai_verify/blindtest/features.py)：char n-gram 哈希桶、列表/标题/代码围栏、tool 名哈希、TTFT。  
...
- VerAI 现有 [`fingerprint.py`](../../ai_verify/monitor/fingerprint.py) 面向**渠道 API**，不是 Cursor 内 Auto；复用题库思想，但会话上下文不同
