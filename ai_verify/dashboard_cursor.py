...
- `ai-verify cursor` / `cursor board`  展示看板
...
def build_view(
    ...
    footer = Text()
    hint = "Ctrl+C 退出" if updated_at else "ai-verify cursor board --watch  常驻刷新"
...
def ensure_imported(since: str = "7d", full: bool = False) -> None:
    from ai_verify.monitor.cursor_usage import CursorUsageImporter
...
def show_board(
    ...
    if period.task_count == 0:
        ...
        console.print("[dim]尝试: ai-verify cursor import --since 30d[/dim]")
        return
