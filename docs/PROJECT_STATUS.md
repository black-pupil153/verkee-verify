...
VerAI 渠道验真 + Cursor Auto Usage MVP + Auto 全量透视主路径 + 手选 train/coverage + **H 融合默认开启** + 标题 summary 回退均已完成。下一里程碑：可选 D/F 探针先验 / ITT / proxy merge；扩手选样本与阈值校准。
...
- 本地语料/模型在 `~/.ai-verify/blindtest/`（不入 git）。
...
最近结果：`119 passed, 1 deselected`（2026-07-13）。
...
9. **可选**：周期探针先验（D/F）、ITT（E）、proxy merge；扩手选样本与阈值校准
...
不要重做 Cursor P0/P1、长检索、G/H/C+B、手选 train/coverage，或 H 默认融合/标题回退。
下一优先：可选 D/F 探针先验 / ITT / proxy merge；或扩手选样本再校准。
