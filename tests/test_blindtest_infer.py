...
from ai_verify.blindtest.corpus import Corpus, CorpusSample
from ai_verify.monitor.blindtest_infer import ensure_task_inferences
from ai_verify.monitor.cursor_usage import CursorUsageImporter, aggregate_task
from ai_verify.providers.cursor import CursorPaths, read_conversation_summaries
...
def test_ensure_task_inferences_writes_without_touching_resolved(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "ai_verify.blindtest.corpus.discover_transcript_files",
...
def test_aggregate_task_auto_infer_hook(tmp_path, monkeypatch):
    ...
    _train_tiny_model(model_path)
    monkeypatch.setattr(
        "ai_verify.monitor.blindtest_infer.DEFAULT_MODEL_PATH", model_path
    )
