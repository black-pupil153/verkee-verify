...
def load_cursor_period(
    ...
    if refresh:
        from ai_verify.dashboard_cursor import ensure_imported
...
def render_cursor_section(
    ...
    if not period_report:
        console.print("  [dim]暂无 Cursor 任务数据，先运行 ai-verify cursor import[/dim]")
        console.print()
        return False
    ...
    console.print(
        "[dim]详情看板: ai-verify cursor --since "
...
def generate(period: str = "weekly") -> None:
    ...
    console.print()
    console.print(f"[bold cyan]AI Verify {period} 报告[/bold cyan]  (最近 {hours} 小时)")
    console.print()
    ...
    if not snapshots:
        console.print("  [dim]该周期内暂无打分数据，先运行 ai-verify score 或 monitor start[/dim]")
        console.print()
    else:
        for s in snapshots:
