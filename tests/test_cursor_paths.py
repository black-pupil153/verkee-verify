...
def test_discover_cursor_paths_defaults(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("verkee_verify.providers.cursor._cursor_home", lambda: cursor_home)
    monkeypatch.setattr(
        "verkee_verify.providers.cursor._cursor_app_support", lambda: app_support
    )
...
def test_run_doctor_minimal(tmp_path, monkeypatch):
    ...
    report = run_doctor(verkee_verify_db=tmp_path / "missing.db")
    ...
    assert report.suggestion.startswith("verkee-verify cursor import")
