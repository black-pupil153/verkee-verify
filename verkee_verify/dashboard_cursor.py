...
- `verkee-verify cursor` / `cursor board`  展示看板
...
def build_view(
    ...
    footer = Text()
    hint = "Ctrl+C 退出" if updated_at else "verkee-verify cursor board --watch  常驻刷新"
...
def ensure_imported(since: str = "7d", full: bool = False) -> None:
    from verkee_verify.monitor.cursor_usage import CursorUsageImporter
...
def show_board(
    ...
    if period.task_count == 0:
        ...
        console.print("[dim]尝试: verkee-verify cursor import --since 30d[/dim]")
        return
