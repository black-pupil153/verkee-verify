...
**Phase 1 已落地并本机跑通**（split + 密封标签 + forced/selective eval）。**Phase 2**：duration_ms、通道消融、`eval-auto`；GT 扩量 = 批量「手选模型」会话（标签自动对齐，禁止 Auto 自标）。Phase 3 微调后置。可选 D/F、ITT、proxy merge — 仅用户点名。
