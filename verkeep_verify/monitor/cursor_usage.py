...
from verkeep_verify.cursor_hook_events import default_hook_event_paths, read_hook_events
from verkeep_verify.cursor_logs import read_log_events
...
def cursor_usage_insights(period: "PeriodUsageReport") -> List[str]:
    ...
    if pending_pct >= 20:
        ...
        insights.append(
            ...
            "可 train 盲测后查看推断轨，或运行 verkeep-verify cursor recommend"
        )
    elif unknown_pct >= 20:
        insights.append(
            f"unknown 请求占 {unknown_pct:.0f}%，可运行 verkeep-verify cursor import --full 补采"
        )
