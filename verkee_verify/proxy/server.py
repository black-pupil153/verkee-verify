...
class ProxyServer:
    ...
    def db(self) -> Database:
        """延迟初始化数据库，避免测试/导入时强制写 ~/.verkee/verify"""
        if self._db is None:
            self._db = Database()
        return self._db
    ...
    def _build_alert_manager(self):
        """根据配置构建报警管理器"""
        from verkee_verify.alerts import AlertManager
    ...
    def _send_alert(self, requested_model, actual_model, anomalies, latency_ms):
        """发送异常报警到控制台 + Webhook"""
        try:
            from verkee_verify.alerts import AlertLevel
...
def start_proxy_server(host: str = "127.0.0.1", port: int = 8080):
    ...
    if not target_url:
        console.print("[red]✗ 请先配置目标 API: verkee-verify config set base_url <url>[/red]")
        return
