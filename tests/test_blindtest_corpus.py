...
from verkee_verify.blindtest.corpus import (
    LabelIndex,
    build_corpus,
    extract_turns,
    load_corpus,
...
def test_task_fingerprint_collision_skips_ambiguous():
    idx = LabelIndex()
    prompt = "You are generating ground-truth Cursor usage for Verkee Verify blindtest (APP-12). " + (
        "x" * 40
    )
...
def test_conversation_overrides_win_over_mislabel(tmp_path):
    ...
    _write_transcript(
        ...
        [
            (
                "You are generating ground-truth Cursor usage for Verkee Verify blindtest (APP-12). "
